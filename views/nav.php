<!--Navigation Start--> 
<div class="container-fluid bg-primary">
    <nav class="container navbar navbar-expand-lg navbar-light ">
  <a class="navbar-brand text-white english" href="index.php">Home</a>
  <button class="  navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
    <span class="navbar-toggler-icon"></span>
  </button>

  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav ml-auto">
      <!-- <li class="nav-item active">
        <a class="nav-link text-white english" href="#">Home <span class="sr-only">(current)</span></a>
      </li> -->

     
      <li class="nav-item">
        <a class="nav-link text-white english" href="#">
        <?php
          if(checkSession("username")){
           echo getSession("username");
          }else{
            echo "Member";
          }
       ?>
        </a>
      </li>

      <li class="nav-item">
        <a class="nav-link text-white english" href="#">NEWS</a>
      </li>
    
      <li class="nav-item">
        <a class="nav-link text-white english" href="#">Positic</a>
      </li>
      <li class="nav-item">
        <a class="nav-link text-white english" href="#">Wars</a>
      </li>
      <li class="nav-item">
        <a class="nav-link text-white english" href="#">IT NEWS</a>
      </li>
      <?php
           if(checkSession("username")){
            echo "
            <a class='nav-link text-white english' href='logout.php'>Logout</a>
          ";
           }else{
            echo "<a class='nav-link text-white english' href='register.php'>Register</a>
            <a class='nav-link text-white english' href='login.php'>Login</a>
            ";
           }
       ?>
      <!-- <li class="nav-item">
        <a class="nav-link text-white english" href="register.php">Register</a>
      </li>
      <li class="nav-item">
        <a class="nav-link text-white english" href="login.php">Login</a>
      </li> -->
      
      <!-- <li class="nav-item dropdown">
        <a class="nav-link dropdown-toggle text-white english" href="#" id="myDD" role="button" data-toggle="dropdown" >
         Members
        </a>
        <div class="dropdown-menu" >
          <a class="dropdown-item" href="#">Register</a>
          <a class="dropdown-item" href="#">Login</a>
          <div class="dropdown-divider"></div>
          <a class="dropdown-item" href="#">Logout</a>
        </div>
      </li> -->
    </ul>
    
    
  </div>
</nav>
    </div>
    <!-- Navication end -->